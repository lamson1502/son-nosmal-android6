...
public class PlaneController extends SingleController implements Contactable {
    ...
    public void getHit(int damage) {
        ...
        System.out.println(((Plane) gameObject).getHP());
    }
    ...
    private void createBullet() {
        if(gift == 0) {
            BulletController bulletController = new BulletController(
                    ...
                    new GameView(Utils.loadImageFromRes("bullet-single.png"))
            );
        ...
        if(gift == 1){
            for(int i = 0; i < 2; i++){
                BulletController bulletController = new BulletController(
                        new Bullet(gameObject.getMiddleX() - i * 10, gameObject.getY()),
                        new GameView(Utils.loadImageFromRes("bullet-single.png")));
                bulletControllers.add(bulletController);
            }
        }
    }
//    private void createDoubleBullet() {
//        BulletController bulletController = new BulletController(
//                new Bullet(gameObject.getMiddleX(), gameObject.getY()),
